...
@version: V1.0.0\n
@file: RunLogisticRegressionWithSGDBinary.py\n
@time: 2019-10-12 9:56\n
@desc: 逻辑回归二元分类——析StumbleUpon数据集，预测网页是暂时性的（ephemeral）或长青的（evergreen），并且调校参数找出最佳参数组合，提高预测准确度
...
def CreateSparkContext():
    ...
    :return:
    '''
    sparkConf = SparkConf().setAppName('RunLogisticRegressionWithSGDBinary').set('spark.ui.showConsoleProgress', 'false')
...
def trainEvaluateModel(trainData, validationData, numIterationsParm, stepSizeParm, miniBatchFractionParm):
    '''
    ...
    print('======================= 训练评估模型 =======================')
    startTime = time()
    model = LogisticRegressionWithSGD.train(trainData, numIterationsParm, stepSizeParm, miniBatchFractionParm)
    AUC = evaluateModel(model, validationData)
    duration = time() - startTime
    print('========== [trainEvaluateModel] >>>> 训练评估模型：使用参数：numIterations=' + str(numIterationsParm) + ', stepSize=' + str(stepSizeParm) + ', miniBatchFraction=' + str(miniBatchFractionParm) + '\n' +
          '\t\t==>> 所需时间=' + str(duration) + ', 结果AUC=' + str(AUC))
    return (AUC, duration, numIterationsParm, stepSizeParm, miniBatchFractionParm, model)
...
def evalParameter(trainData, validationData, evalParm, numIterationsList, stepSizeList, miniBatchFractionList):
    ...
    # 转换为Pandas DataFrame
    df = pd.DataFrame(metrics, index=indexList, columns=['AUC', 'duration', 'numIterations', 'stepSize', 'miniBatchFraction', 'model'])
    # 显示图形
...
def parameterEval(trainData, validationData):
    ...
    # 评估 numIterations 参数
    evalParameter(trainData, validationData, 'numIterations',
                  numIterationsList=[3, 5, 10, 15, 20, 60, 100],
                  stepSizeList=[10],
                  miniBatchFractionList=[1])
    # 评估 stepSize 参数
...
def evalAllParameter(trainData, validationData, numIterationsList, stepSizeList, miniBatchFractionList):
    ...
    # 显示调校后最佳参数组合
    print('========== [evalAllParameter] >>>> 调校后最佳参数组合：numIterations=' + str(bestParameter[2]) + ', stepSize=' + str(bestParameter[3]) + ', miniBatchFraction=' + str(bestParameter[4]) + '\n' +
...
if __name__ == '__main__':
    print('@@@@@@@@@@@@@@@@@@@@@@@@@ Start RunLogisticRegressionWithSGDBinary @@@@@@@@@@@@@@@@@@@@@@@@@')
