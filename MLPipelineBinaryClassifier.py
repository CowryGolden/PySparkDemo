...
@desc: Spark ML Pipeline 机器学习流程二元分类【TrainValidationSplit、CrossValidation】，以及使用随机森林分类器（RandomForestClassifier）
'''
import  os, sys
import pandas as pd
...
if __name__ == '__main__':
    ...
    # df.printSchema()
    # 将 df 按照 7:3 的比例分成 train_df（训练数据） 与 test_df（测试数据），并暂存在内存中
    ...
    print('========== [Main] >>>> 使用二元分类评估器验证评估模型，使用 testData 评估预测模型，结果AUC=' + str(auc))

    print('@@@@@@@@@@@@@@@@@@@@@@@@@ 使用 DecisionTreeClassifier 决策树分类器和 TrainValidationSplit 进行模型训练和验证，并找出最佳模型阶段 @@@@@@@@@@@@@@@@@@@@@@@@@')
    # paramGrid = ParamGridBuilder().addGrid(dt.impurity, ['gini', 'entropy']).addGrid(dt.maxDepth, [5, 10, 15]).addGrid(dt.maxBins, [10, 15, 20]).build()  # 执行模型参数训练 2*3*3=18次
    # tvs = TrainValidationSplit(estimator=dt, evaluator=evaluator, estimatorParamMaps=paramGrid, trainRatio=0.8)  # 创建模型训练验证对象；参数 trainRatio=0.8 表示：训练验证前会将数据按照 8:2 的比例分成训练数据与验证数据
    # tvsPipline = Pipeline(stages=[categoryIndexer, encoder, assembler, tvs])  # 建立模型训练 Pipeline 流程
    # tvsPiplineModel = tvsPipline.fit(train_df)  # 生成训练后的模型
    # bestModel = tvsPiplineModel.stages[3].bestModel
    # # print('========== [Main] >>>> 查看训练完成后的最佳决策树模型规则：')
    # # print(bestModel.toDebugString[:500])  # 只显示前500个字符
    ...
    # cv = CrossValidator(estimator=dt, evaluator=evaluator, estimatorParamMaps=paramGrid, numFolds=3)
    # cvPipline = Pipeline(stages=[categoryIndexer, encoder, assembler, cv])
    # cvPiplineModel = cvPipline.fit(train_df)
    # bestModel1 = cvPiplineModel.stages[3].bestModel
    # # print('========== [Main] >>>> 查看训练完成后的最佳决策树模型规则：')
    # # print(bestModel1.toDebugString[:500])  # 只显示前500个字符
    # predictions = cvPiplineModel.transform(test_df)
    # auc = evaluator.evaluate(predictions)
    ...
    # paramGrid1 = ParamGridBuilder().addGrid(rf.impurity, ['gini', 'entropy']).addGrid(rf.maxDepth, [5, 10, 15]).addGrid(rf.maxBins, [10, 15, 20]).addGrid(rf.numTrees, [10, 20, 30]).build()
    # rftvs = TrainValidationSplit(estimator=rf, evaluator=evaluator, estimatorParamMaps=paramGrid1, trainRatio=0.8)
    # rftvsPipeline = Pipeline(stages=[categoryIndexer, encoder, assembler, rftvs])
    # rftvsPipelineModel = rftvsPipeline.fit(train_df)
    # predictions = rftvsPipelineModel.transform(test_df)
    # auc = evaluator.evaluate(predictions)
    ...
    # paramGrid1 = ParamGridBuilder().addGrid(rf.impurity, ['gini', 'entropy']).addGrid(rf.maxDepth, [5, 10, 15]).addGrid(rf.maxBins, [10, 15, 20]).addGrid(rf.numTrees, [10, 20, 30]).build()
    # rfcv = CrossValidator(estimator=dt, evaluator=evaluator, estimatorParamMaps=paramGrid1, numFolds=3)
    # rfcvPipeline = Pipeline(stages=[categoryIndexer, encoder, assembler, rfcv])
    # rftvsPipelineModel = rfcvPipeline.fit(train_df)
    # predictions = rftvsPipelineModel.transform(test_df)
    # auc = evaluator.evaluate(predictions)
